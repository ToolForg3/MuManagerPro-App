...
(function () {
  const OFFICIAL_HASH = '2A8DC80A899920EF2E59AF76D3340AD1B9F4541792A02EE57ADD71726C71E83E';
  const OFFICIAL_SIZE_BYTES = 66633722; // ~63.54 MB (v2.1.8 Build 117)
    ...
    if (isMatch) {
      resultCard.innerHTML = `
        <div class="result-header">
          ...
          <span>INTEGRIDAD CRIPTOGRÁFICA VERIFICADA — v2.1.8 OFICIAL</span>
        </div>
        <div style="font-size: 0.85rem; color: var(--text-main); display: flex; flex-direction: column; gap: 4px;">
          ...
          <p style="margin-top: 6px; color: var(--text-secondary);">El archivo analizado coincide bit a bit con la compilación oficial distribuida v2.1.8. Su integridad de descarga está confirmada.</p>
        </div>
      `;
    } else {
      resultCard.innerHTML = `
        <div class="result-header">
          ...
          <span>ALERTA: HASH NO COINCIDE CON LA VERSIÓN OFICIAL v2.1.8</span>
        </div>
        <div style="font-size: 0.85rem; color: var(--text-main); display: flex; flex-direction: column; gap: 4px;">
          ...
          <div><strong>Hash oficial esperado (v2.1.8):</strong></div>
          ...
          <p style="margin-top: 6px; color: var(--text-secondary);">El archivo analizado difiere de la compilación v2.1.8 oficial. Por seguridad, verifica la versión instalada o descarga el APK oficial provisto en este sitio web.</p>
        </div>
      `;
    }
  }
