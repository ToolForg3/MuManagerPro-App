...
(function () {
  const OFFICIAL_HASH = 'A9F16A281627E82D9AE12A433A91B86B84B7F97594D9BD412CAE34C268460450';
  const OFFICIAL_SIZE_BYTES = 66630750; // ~63.54 MB (v2.1.7 Build 117)
    ...
    if (isMatch) {
      resultCard.innerHTML = `
        <div class="result-header">
          ...
          <span>INTEGRIDAD CRIPTOGRÁFICA VERIFICADA — v2.1.7 OFICIAL</span>
        </div>
        <div style="font-size: 0.85rem; color: var(--text-main); display: flex; flex-direction: column; gap: 4px;">
          ...
          <p style="margin-top: 6px; color: var(--text-secondary);">El archivo analizado coincide bit a bit con la compilación oficial distribuida v2.1.7. Su integridad de descarga está confirmada.</p>
        </div>
      `;
    } else {
      resultCard.innerHTML = `
        <div class="result-header">
          ...
          <span>ALERTA: HASH NO COINCIDE CON LA VERSIÓN OFICIAL v2.1.7</span>
        </div>
        <div style="font-size: 0.85rem; color: var(--text-main); display: flex; flex-direction: column; gap: 4px;">
          ...
          <div><strong>Hash oficial esperado (v2.1.7):</strong></div>
          ...
          <p style="margin-top: 6px; color: var(--text-secondary);">El archivo analizado difiere de la compilación v2.1.7 oficial. Por seguridad, verifica la versión instalada o descarga el APK oficial provisto en este sitio web.</p>
        </div>
      `;
    }
  }
