...
(function () {
  const OFFICIAL_HASH = '7E97C821A2C5B0C989EFF8FA9504E8A4E0C2E5D838CA22AAD11A6CC130C1E9B4';
  const OFFICIAL_SIZE_BYTES = 66628854; // ~63.54 MB (v2.1.5 Build 116)
    ...
    if (isMatch) {
      resultCard.innerHTML = `
        <div class="result-header">
          ...
          <span>INTEGRIDAD CRIPTOGRÁFICA VERIFICADA — v2.1.5 OFICIAL</span>
        </div>
        <div style="font-size: 0.85rem; color: var(--text-main); display: flex; flex-direction: column; gap: 4px;">
          ...
          <p style="margin-top: 6px; color: var(--text-secondary);">El archivo analizado coincide bit a bit con la compilación oficial distribuida v2.1.5. Su integridad de descarga está confirmada.</p>
        </div>
      `;
    } else {
      resultCard.innerHTML = `
        <div class="result-header">
          ...
          <span>ALERTA: HASH NO COINCIDE CON LA VERSIÓN OFICIAL v2.1.5</span>
        </div>
        <div style="font-size: 0.85rem; color: var(--text-main); display: flex; flex-direction: column; gap: 4px;">
          ...
          <div><strong>Hash oficial esperado (v2.1.5):</strong></div>
          ...
          <p style="margin-top: 6px; color: var(--text-secondary);">El archivo analizado difiere de la compilación v2.1.5 oficial. Por seguridad, verifica la versión instalada o descarga el APK oficial provisto en este sitio web.</p>
        </div>
      `;
    }
  }
